...
static AsebaNativeFunctionDescription AsebaNativeDescription__system_settings_read = { "_system.settings.read",
	"Read a setting",
...
static AsebaNativeFunctionDescription AsebaNativeDescription__system_settings_write = { "_system.settings.write",
	"Write a setting",
	{ { 1, "address" }, { 1, "value" }, { 0, 0 } } };
...
unsigned char aseba_flash[PAGE_PER_CHUNK][INSTRUCTIONS_PER_PAGE * 2]
	__attribute__((space(prog), aligned(INSTRUCTIONS_PER_PAGE * 2), section(".aseba_bytecode"),
		address(0x15800
			- 0x800 /* bootloader */ - 0x400 /* settings */ - NUMBER_OF_CHUNK * 0x400L * PAGE_PER_CHUNK) /*, noload*/));
...
void init_aseba_and_can(void)
{
	...
	can_init((can_frame_received_callback)AsebaCanFrameReceived,
		AsebaCanFrameSent,
		DMA_CAN_RX,
		DMA_CAN_TX,
		CAN_SELECT_MODE,

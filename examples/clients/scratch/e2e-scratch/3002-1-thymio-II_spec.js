...
.after(function(err, res, body) {
	...
	.get(ASEBASCRATCH + 'events?todo=60')
        //.inspectBody()
	.expectStatus(200)
...
frisby.create('Set then reset the user variable Unused, using POST & JSON')
.get(ASEBASCRATCH + THYMIO2 + '/Unused')
.expectStatus(200)
.expectHeader('Content-Type', 'application/json')
.expectJSONLength(1)
.afterJSON(function (body) {
    var old_flag = body[0]
    frisby.create('Set the value of Unused')
    .post(ASEBASCRATCH + THYMIO2 + '/Unused', [143], {json: true})
    .expectStatus(204)
    .after(function(err, res, body) {
        frisby.create('Get the value of Unused')
        .get(ASEBASCRATCH + THYMIO2 + '/Unused')
        .expectStatus(200)
        .expectJSON([143])
        .afterJSON(function (body) {
            frisby.create('Set the value of Unused')
            .post(ASEBASCRATCH + THYMIO2 + '/Unused', [old_flag], {json: true})
            .expectStatus(204)
            .after(function(err, res, body) {
                frisby.create('Get the value of Unused')
                .get(ASEBASCRATCH + THYMIO2 + '/Unused')

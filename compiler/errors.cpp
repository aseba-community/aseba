...
namespace Aseba
{
	...
	ErrorMessages::ErrorMessages()
	{
		...
		error_map[ERROR_STACK_OVERFLOW] = L"Execution stack will overflow, check for any recursive subroutine call and "
										  L"cut long mathematical expressions";
		...
		error_map[ERROR_INVALID_IDENTIFIER] =
			L"Identifiers must begin with _ or an alphanumeric character, found unicode character 0x%0 instead";
		...
		// parser.cpp
		error_map[ERROR_INTERNAL] =
			L"Internal compiler error, please report a bug containing the source which triggered this error";
		...
		error_map[ERROR_EVENT_ARG_TOO_BIG] =
			L"Event %0 needs an array of size %1 or smaller, but one of size %2 is passed";
		...
		error_map[ERROR_FUNCTION_WRONG_ARG_SIZE_TEMPLATE] =
			L"Argument %0 (%1) of function %2 is of size %3, while a previous instance of the template parameter was "
			L"of size %4";
	...
	Error TranslatableError::toError() { return Error(pos, message); }

	void TranslatableError::setTranslateCB(ErrorMessages::ErrorCallback newCB) { translateCB = newCB; }

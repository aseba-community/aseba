...
struct BytecodeVector : std::deque<BytecodeElement> {
    ...
    unsigned lastLine{0};       //!< last line added, normally equal *this[this->size()-1].line, but
                                //!< may differ for instance on loops
...
struct SourcePos {
    ...
    SourcePos(unsigned character, unsigned row, unsigned column)
        : character(character - 1), row(row), column(column - 1) {
        valid = true;
    }
    SourcePos() = default;
...
public:
    ...
    struct SubroutineDescriptor {
        ...
        SubroutineDescriptor(std::wstring name, unsigned address, unsigned line)
            : name(std::move(name)), address(address), line(line) {}
    };
...
public:
    ...
    bool compile(std::wistream& source, BytecodeVector& bytecode, unsigned& allocatedVariablesCount,
                 Error& errorDescription, std::wostream* dump = nullptr);
    void setTranslateCallback(ErrorMessages::ErrorCallback newCB) {
        TranslatableError::setTranslateCB(newCB);
    }
...
protected:
    ...
    unsigned endVariableIndex;                      //!< (endMemory - endVariableIndex) is pointing to the first free
                                                    //!< variable at the end

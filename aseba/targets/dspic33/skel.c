...
static AsebaNativeFunctionDescription AsebaNativeDescription__system_reboot = {"_system.reboot",
...
static AsebaNativeFunctionDescription AsebaNativeDescription__system_settings_flash = {"_system.settings.flash",
...
// Put everything in the same section, so we are 100% sure that the linker will put them
// contiguously. Force the address, since the linker sometimes put it in the middle of the code
#define NUMBER_OF_CHUNK 26
unsigned char aseba_flash[PAGE_PER_CHUNK][INSTRUCTIONS_PER_PAGE * 2]
    __attribute__((space(prog), aligned(INSTRUCTIONS_PER_PAGE * 2), section(".aseba_bytecode"),
                   address(0x15800 - 0x800 /* bootloader */ - 0x400 /* settings */ -
                           NUMBER_OF_CHUNK * 0x400L * PAGE_PER_CHUNK) /*, noload*/));
unsigned char aseba_flash1[PAGE_PER_CHUNK][INSTRUCTIONS_PER_PAGE * 2]
    __attribute__((space(prog), aligned(INSTRUCTIONS_PER_PAGE * 2), section(".aseba_bytecode") /*, noload*/));
unsigned char aseba_flash2[PAGE_PER_CHUNK][INSTRUCTIONS_PER_PAGE * 2]
    __attribute__((space(prog), aligned(INSTRUCTIONS_PER_PAGE * 2), section(".aseba_bytecode") /*, noload*/));
unsigned char aseba_flash3[PAGE_PER_CHUNK][INSTRUCTIONS_PER_PAGE * 2]
    __attribute__((space(prog), aligned(INSTRUCTIONS_PER_PAGE * 2), section(".aseba_bytecode") /*, noload*/));
unsigned char aseba_flash4[PAGE_PER_CHUNK][INSTRUCTIONS_PER_PAGE * 2]
    __attribute__((space(prog), aligned(INSTRUCTIONS_PER_PAGE * 2), section(".aseba_bytecode") /*, noload*/));
unsigned char aseba_flash5[PAGE_PER_CHUNK][INSTRUCTIONS_PER_PAGE * 2]
    __attribute__((space(prog), aligned(INSTRUCTIONS_PER_PAGE * 2), section(".aseba_bytecode") /*, noload*/));
unsigned char aseba_flash6[PAGE_PER_CHUNK][INSTRUCTIONS_PER_PAGE * 2]
    __attribute__((space(prog), aligned(INSTRUCTIONS_PER_PAGE * 2), section(".aseba_bytecode") /*, noload*/));
unsigned char aseba_flash7[PAGE_PER_CHUNK][INSTRUCTIONS_PER_PAGE * 2]
    __attribute__((space(prog), aligned(INSTRUCTIONS_PER_PAGE * 2), section(".aseba_bytecode") /*, noload*/));
unsigned char aseba_flash8[PAGE_PER_CHUNK][INSTRUCTIONS_PER_PAGE * 2]
    __attribute__((space(prog), aligned(INSTRUCTIONS_PER_PAGE * 2), section(".aseba_bytecode") /*, noload*/));
unsigned char aseba_flash9[PAGE_PER_CHUNK][INSTRUCTIONS_PER_PAGE * 2]
    __attribute__((space(prog), aligned(INSTRUCTIONS_PER_PAGE * 2), section(".aseba_bytecode") /*, noload*/));
unsigned char aseba_flash10[PAGE_PER_CHUNK][INSTRUCTIONS_PER_PAGE * 2]
    __attribute__((space(prog), aligned(INSTRUCTIONS_PER_PAGE * 2), section(".aseba_bytecode") /*, noload*/));
unsigned char aseba_flash11[PAGE_PER_CHUNK][INSTRUCTIONS_PER_PAGE * 2]
    __attribute__((space(prog), aligned(INSTRUCTIONS_PER_PAGE * 2), section(".aseba_bytecode") /*, noload*/));
unsigned char aseba_flash12[PAGE_PER_CHUNK][INSTRUCTIONS_PER_PAGE * 2]
    __attribute__((space(prog), aligned(INSTRUCTIONS_PER_PAGE * 2), section(".aseba_bytecode") /*, noload*/));
unsigned char aseba_flash13[PAGE_PER_CHUNK][INSTRUCTIONS_PER_PAGE * 2]
    __attribute__((space(prog), aligned(INSTRUCTIONS_PER_PAGE * 2), section(".aseba_bytecode") /*, noload*/));
unsigned char aseba_flash14[PAGE_PER_CHUNK][INSTRUCTIONS_PER_PAGE * 2]
    __attribute__((space(prog), aligned(INSTRUCTIONS_PER_PAGE * 2), section(".aseba_bytecode") /*, noload*/));
unsigned char aseba_flash15[PAGE_PER_CHUNK][INSTRUCTIONS_PER_PAGE * 2]
    __attribute__((space(prog), aligned(INSTRUCTIONS_PER_PAGE * 2), section(".aseba_bytecode") /*, noload*/));
unsigned char aseba_flash16[PAGE_PER_CHUNK][INSTRUCTIONS_PER_PAGE * 2]
    __attribute__((space(prog), aligned(INSTRUCTIONS_PER_PAGE * 2), section(".aseba_bytecode") /*, noload*/));
unsigned char aseba_flash17[PAGE_PER_CHUNK][INSTRUCTIONS_PER_PAGE * 2]
    __attribute__((space(prog), aligned(INSTRUCTIONS_PER_PAGE * 2), section(".aseba_bytecode") /*, noload*/));
unsigned char aseba_flash18[PAGE_PER_CHUNK][INSTRUCTIONS_PER_PAGE * 2]
    __attribute__((space(prog), aligned(INSTRUCTIONS_PER_PAGE * 2), section(".aseba_bytecode") /*, noload*/));
unsigned char aseba_flash19[PAGE_PER_CHUNK][INSTRUCTIONS_PER_PAGE * 2]
    __attribute__((space(prog), aligned(INSTRUCTIONS_PER_PAGE * 2), section(".aseba_bytecode") /*, noload*/));
unsigned char aseba_flash20[PAGE_PER_CHUNK][INSTRUCTIONS_PER_PAGE * 2]
    __attribute__((space(prog), aligned(INSTRUCTIONS_PER_PAGE * 2), section(".aseba_bytecode") /*, noload*/));
unsigned char aseba_flash21[PAGE_PER_CHUNK][INSTRUCTIONS_PER_PAGE * 2]
    __attribute__((space(prog), aligned(INSTRUCTIONS_PER_PAGE * 2), section(".aseba_bytecode") /*, noload*/));
unsigned char aseba_flash22[PAGE_PER_CHUNK][INSTRUCTIONS_PER_PAGE * 2]
    __attribute__((space(prog), aligned(INSTRUCTIONS_PER_PAGE * 2), section(".aseba_bytecode") /*, noload*/));
unsigned char aseba_flash23[PAGE_PER_CHUNK][INSTRUCTIONS_PER_PAGE * 2]
    __attribute__((space(prog), aligned(INSTRUCTIONS_PER_PAGE * 2), section(".aseba_bytecode") /*, noload*/));
unsigned char aseba_flash24[PAGE_PER_CHUNK][INSTRUCTIONS_PER_PAGE * 2]
    __attribute__((space(prog), aligned(INSTRUCTIONS_PER_PAGE * 2), section(".aseba_bytecode") /*, noload*/));
unsigned char aseba_flash25[PAGE_PER_CHUNK][INSTRUCTIONS_PER_PAGE * 2]
    __attribute__((space(prog), aligned(INSTRUCTIONS_PER_PAGE * 2), section(".aseba_bytecode") /*, noload*/));

// Saveguard the bootloader (2 pages)
unsigned char __bootloader[INSTRUCTIONS_PER_PAGE * 2 * 2]
    __attribute((space(prog), section(".boot"), noload, address(0x15800 - 0x800)));

// Put the settings page at a fixed position so it's independant of linker mood.
unsigned char aseba_settings_flash[INSTRUCTIONS_PER_PAGE * 2]
...
error_handler(const char* file, int line, int id, void* __attribute((unused)) arg) {
...
void init_aseba_and_can(void) {
    ...
    can_init((can_frame_received_callback)AsebaCanFrameReceived, AsebaCanFrameSent, DMA_CAN_RX, DMA_CAN_TX,
             CAN_SELECT_MODE, 1000, PRIO_CAN);
    AsebaCanInit(vmState.nodeId, (AsebaCanSendFrameFP)can_send_frame, can_is_frame_room, received_packet_dropped,
                 sent_packet_dropped, sendQueue, SEND_QUEUE_SIZE, recvQueue, RECV_QUEUE_SIZE);
    AsebaVMInit(&vmState);
...
void __attribute((noreturn)) run_aseba_main_loop(void) {
    while(1) {
        ...
        // Either we are in step by step, so we go to sleep until further commands, or we are not
        // executing an event, and so we have nothing to do.
        if(AsebaMaskIsSet(vmState.flags, ASEBA_VM_STEP_BY_STEP_MASK) ||
           AsebaMaskIsClear(vmState.flags, ASEBA_VM_EVENT_ACTIVE_MASK)) {
            ...
            // Warning, I'm doing some nasty things with the IPL bits (forcing the value, without
            // safeguard) Second warning: It doesn't disable level 7 interrupt. So if you set an
            // event inside a level 7 interrupt It may get delayed until next interrupt
            asm __volatile__("mov #SR, w0 \r\n"
                             ...
                             "cp0 w0\r\n"
                             "bra z, 1f \r\n"
                             "rcall _clock_idle\r\n"  // Powersave WITH interrupt disabled. It
                                                      // works, read section 6.2.5 of dsPIC manual
                             "1: \r\n"
                             "mov #SR, w0 \r\n"
                             "mov #0x1F, w1\r\n"
                             "and.b w1, [w0],[w0] \r\n"  // enable interrupts
            ...
            // Why putting "x" as constrain register. Because it's w8-w9, so it's preserved accross
            // function call we do a rcall, so we must clobber w0-w7
            ...
            // FIXME: do we want to kill execution upon local events? that would be consistant so
            // Steph votes yes, but we may discuss further
            if(i &&
               !(AsebaMaskIsSet(vmState.flags, ASEBA_VM_STEP_BY_STEP_MASK) &&
                 AsebaMaskIsSet(vmState.flags, ASEBA_VM_EVENT_ACTIVE_MASK))) {
                // that is the same as (thx de Morgan)
                // if(i && (AsebaMaskIsClear(vmState.flags, ASEBA_VM_STEP_BY_STEP_MASK) ||
                // AsebaMaskIsClear(vmState.flags, ASEBA_VM_EVENT_ACTIVE_MASK))) {
                i--;
                CLEAR_EVENT(i);

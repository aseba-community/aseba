...
static void can_rx_cb(const can_frame* frame) {
    ...
    // we do a nasty cast because the two data structure are compatible, i.e. they have the same
    // leading members
...
static bool uart_rx_cb(int __attribute((unused)) uart_id, unsigned char data, void* __attribute((unused)) user_data) {
    ...
    // TODO: in case of bug, we might want to test for overflow here, but it should never happen if
    // higher layer conform to specification
...
static void received_packet_dropped(void) {
    // light a LED, we have dropped an aseba message from the CAN because we were not able to send
    // previous ones fast enough on UART
    gpio_write(LED1, 0);
}

static void sent_packet_dropped(void) {
    // light a LED, the packet is in fact not dropped because UART has flow control and the
    // translator will try again later
    gpio_write(LED1, 0);
}

void __attribute__((noreturn)) error_handler(const char __attribute((unused)) * file, int __attribute((unused)) line,
                                             int id, void* __attribute((unused)) arg) {
    OUTPUT_ERROR(id);
}
...
int main(void) {
    ...
    AsebaCanInit(0, (AsebaCanSendFrameFP)can_send_frame, can_is_frame_room, received_packet_dropped,

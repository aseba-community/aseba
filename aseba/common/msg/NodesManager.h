...
//! For now, it does not support the disconnection of a whole network nor the update of the
//! description of any node
class NodesManager {
protected:
...
public:
    ...
    //! Return the id of the node corresponding to name and set ok to true, if provided; if invalid,
    //! return 0xFFFFFFFF and set ok to false. If several nodes have this name, either return
    //! preferedId if it exists otherwise return the first found.
    unsigned getNodeId(const std::wstring& name, unsigned preferedId = 0, bool* ok = nullptr) const;
    //! Return the description of a node and set ok to true, if provided; if invalid, return 0 and
    //! set ok to false
    const TargetDescription* getDescription(unsigned nodeId, bool* ok = nullptr) const;
    //! Return the position of a variable and set ok to true, if provided; if invalid, return
    //! 0xFFFFFFFF and set ok to false
    unsigned getVariablePos(unsigned nodeId, const std::wstring& name, bool* ok = nullptr) const;
    //! Return the length of a variable and set ok to true, if provided; if invalid, return
    //! 0xFFFFFFFF and set ok to false
...
protected:
    //! Check if a node description has been fully received, and if so, call the
    //! nodeDescriptionReceived() virtual function
    ...
    //! Virtual function that is called when a node is first seen, meaning its description has been
    //! fully received
    virtual void nodeDescriptionReceived(unsigned nodeId) {}

    //! Virtual function that is called when a node is connected, we know its description has been
    //! fully received
    virtual void nodeConnected(unsigned nodeId) {}

    //! Virtual function that is called when a node is explicitly connected or has not been seen for
    //! a while

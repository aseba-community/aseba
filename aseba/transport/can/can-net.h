...
/*! Init the CAN connection.
    ...
    @param isFrameRoomFP pointer to a function that returns if the data layer is ready to send
   frames
    @param receivedPacketDroppedFP pointer to a function that is called when a received packet has
   been dropped, for various reasons but mostly related to insufficient memory
    @param sentPacketDroppedFP pointer to a function that is called when a sent packet has been
   dropped (AsebaCanSend() returned 0), for various reasons but mostly related to insufficient
   memory
...
*/
void AsebaCanInit(uint16_t id, AsebaCanSendFrameFP sendFrameFP, AsebaCanIntVoidFP isFrameRoomFP,
                  AsebaCanVoidVoidFP receivedPacketDroppedFP, AsebaCanVoidVoidFP sentPacketDroppedFP,
                  CanFrame* sendQueue, size_t sendQueueSize, CanFrame* recvQueue, size_t recvQueueSize);
...
/*! Copy data from a received packet to the caller-provided buffer.
    ...
    @param size makimum number of byte to copy. Actual number of byte might be smaller if packet was
   smaller. If packet was bigger, the rest of the data are dropped.

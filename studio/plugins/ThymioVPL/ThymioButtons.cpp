...
namespace Aseba
{
	...
	{		
		...
		else // buttonType == THYMIO_RECTANGULAR_BUTTON 
	...
	ThymioButton::ThymioButton(bool eventButton, qreal scale, bool up, QGraphicsItem *parent) : 
	...
	QPixmap ThymioButton::image(bool on)
	{
		...
		QSvgRenderer *svgRenderer = renderer();
		if( svgRenderer ) 
	...
	ThymioButtonSet::ThymioButtonSet(int row, QGraphicsItem *parent) : 
		QGraphicsObject(parent),
		eventButton(0),
		actionButton(0),
	...
	{ 
		...
		deleteButton->setPos(896, 168);
		
		addButton = new ThymioAddButton(this);
		addButton->setPos(500, 368);
	...
	void ThymioButtonSet::paint (QPainter * painter, const QStyleOptionGraphicsItem * option, QWidget * widget)
	{
		...
		painter->drawRoundedRect(0, 0, 1000, 336, 5, 5);

		painter->setBrush(QColor(167, 151, 128, alpha));
		painter->drawRect(340, 143, 55, 55);

		QPointF pts[3];
		pts[0] = QPointF(394.5, 118);
		pts[1] = QPointF(394.5, 218);
	...
	void ThymioButtonSet::mouseMoveEvent( QGraphicsSceneMouseEvent *event )
	{
		...
		QPixmap img = image();
		drag->setMimeData(mime);
		drag->setPixmap(img.scaled(1000*scale(),336*scale()));
		drag->setHotSpot(QPoint(500*scale(), 168*scale()));
	...
	// Thymio Push Button
	ThymioPushButton::ThymioPushButton(QWidget *parent) : 
		QPushButton(parent), 
		thymioButton(0),
		prevSpan(128)
	{ 
	...
	void ThymioPushButton::setThymioButton(ThymioButton *button) 
	{ 
		thymioButton = button; 
		setIcon(button->image()); 
		setToolTip(QString("%0 %1").arg(button->getName()).arg(button->getType()));
	}

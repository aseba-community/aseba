...
void VariableOrEventHandler::handleRequest(HttpRequest* request, const std::vector<std::string>& tokens)
{
		 ...
		 ++iter)
	{
		...
		else if (request->getMethod().find("POST") == 0 || tokens.size() >= 3)
		{ // set variable value
...
std::map<std::string, std::string> const FileHandler::suffixMapping = { { "css", "text/css" },
	{ "gif", "image/gif" },
	{ "htm", "text/html" },
	{ "html", "text/html" },
	{ "jpg", "image/jpg" },

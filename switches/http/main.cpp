...
int main(int argc, char* argv[])
{
	...
	do
	{
		try
		{
			Aseba::HttpInterface network(
				dashel_target_list, http_port, aseba_port, Kiterations > 0 ? 1000 * Kiterations : 5, dump, verbose);
			...
			for (auto nodeId : network.allNodeIds())
				try
				{
					...
					{
						const char* failsafe = "<!DOCTYPE aesl-source><network><keywords flag=\"true\"/><node "

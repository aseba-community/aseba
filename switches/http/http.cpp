...
namespace Aseba
{
	...
	HttpInterface::HttpInterface(const strings& targets, const std::string& http_port, const std::string& aseba_port,
		const int iterations, bool dump, bool verbose) :
	...
	void HttpInterface::nodeDescriptionReceived(unsigned targetId)
	{
		...
		if (nodeProgramsSent.find(targetId) == nodeProgramsSent.end()
			&& nodeProgram.find(targetId) != nodeProgram.end())
	...
	void HttpInterface::incomingDataHTTP(Stream* stream)
	{
		...
		HttpRequest* req =
			new HttpRequest; // [promise] we will eventually delete req in sendAvailableResponses, unscheduleResponse, or stream shutdown
	...
	void HttpInterface::incomingUserMsg(const UserMessage* userMsg)
	{
		...
		if (eventSubscriptions.size() > 0)
		{
			...
			for (StreamEventSubscriptionMap::iterator subscriber = eventSubscriptions.begin();
				 subscriber != eventSubscriptions.end();
				 ++subscriber)
			{
			...
			else // (do_one_node)
			{
				...
				if (!seen_named_variables)
				{
					...
					for (vector<Aseba::TargetDescription::NamedVariable>::const_iterator i(
						 ...
						 i != description.namedVariables.end();
						 ++i)
	...
	void HttpInterface::aeslLoad(const unsigned nodeId, xmlDoc* doc)
	{
			...
			for (int i = 0; i < (nodeset ? nodeset->nodeNr : 0); ++i)
			{
				...
				if (name && size)
				{
					...
					if (eventSize > ASEBA_MAX_EVENT_ARG_SIZE)
					{
						...
						commonDefinitions[nodeId].events.push_back(
							NamedValue(UTF8ToWString((const char*)name), eventSize));
				}
				xmlFree(name);
				xmlFree(size);
			}
		...
		if ((obj = xmlXPathEvalExpression(BAD_CAST "/network/node", context)))
		{
			...
			for (int i = 0; i < (nodeset ? nodeset->nodeNr : 0); ++i)
			{
				...
				else
				{
					...
					unsigned preferredId =
						nodeToAeslIdSubstitutions[nodeId] ? nodeToAeslIdSubstitutions[nodeId] : nodeId;
		...
		{
			wcerr << noNodeCount
				  << " scripts have no corresponding nodes in the current network and have not been loaded." << endl;
		}
	}
	...
	void HttpInterface::addHeaders(HttpRequest* req, strings& outheaders) { req->outheaders = outheaders; }

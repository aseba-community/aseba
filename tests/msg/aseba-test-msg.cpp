...
void testMessage(function<void(T&)> initFunc, initializer_list<function<void(T&)>> modifyFuncs, Args&&... args) {
    ...
    for(auto& modifyFunc : modifyFuncs) {
        ...
        if(*m1 == *m2) {
            cerr << "Message type " << typeid(T).name() << " are still equal while content changed (function " << count
                 << ") after serialization" << endl;
            throw logic_error("Serialization failed");
        }
...
int main() {
    ...
    testMessageNoInit<UserMessage>({[](UserMessage& m) { m.data[0] = 4; }, [](UserMessage& m) { m.data[1] = 3; },
                                    [](UserMessage& m) { m.data[2] = 2; }},
                                   0, VariablesDataVector{1, 2, 3});

    testMessage<BootloaderDescription>(
        [](BootloaderDescription& m) {
            m.pageSize = 128;
            m.pagesStart = 10;
            m.pagesCount = 20;
        },
        {[](BootloaderDescription& m) { m.pageSize = 64; }, [](BootloaderDescription& m) { m.pagesStart = 20; },
         [](BootloaderDescription& m) { m.pagesCount = 10; }});

    testMessage<BootloaderDataRead>(
        [](BootloaderDataRead& m) {
            m.data = {{1, 2, 3, 4}};
        },
        {[](BootloaderDataRead& m) { m.data[0] = 4; }, [](BootloaderDataRead& m) { m.data[1] = 3; },
         [](BootloaderDataRead& m) { m.data[2] = 2; }, [](BootloaderDataRead& m) { m.data[3] = 1; }});

    testMessage<BootloaderAck>(
        [](BootloaderAck& m) {
            m.errorCode = BootloaderAck::ErrorCode::PROGRAMMING_FAILED;
            m.errorAddress = 0xff;
        },
        {[](BootloaderAck& m) {
            m.errorCode = BootloaderAck::ErrorCode::SUCCESS;
            m.errorAddress = 0;
        }});

    testMessageNoInit<ListNodes>({[](ListNodes& m) { m.version = 1; }});

    testMessageNoInit<NodePresent>({[](NodePresent& m) { m.version = 1; }});

    testMessageNoInit<GetDescription>({[](GetDescription& m) { m.version = 1; }});

    testMessage<GetNodeDescription>(
        [](GetNodeDescription& m) { m.dest = 1; },
        {[](GetNodeDescription& m) { m.dest = 3; }, [](GetNodeDescription& m) { m.version = 1; }});
    ...
    testMessage<Description>(
        ...
        {[](Description& m) { m.name = L"titi"; }, [](Description& m) { m.protocolVersion = 1; },
         [](Description& m) { m.bytecodeSize = 512; }, [](Description& m) { m.stackSize = 32; },
    ...
    testMessage<NamedVariableDescription>(
        ...
        {[](NamedVariableDescription& m) { m.name = L"titi"; }, [](NamedVariableDescription& m) { m.size = 1; }});
    ...
    testMessage<Variables>(
        ...
        {[](Variables& m) { m.start = 20; }, [](Variables& m) { m.variables[0] = 3; },
         [](Variables& m) { m.variables[1] = 4; }, [](Variables& m) { m.variables.push_back(5); }});
    ...
    testMessage<ArrayAccessOutOfBounds>(
        ...
        {[](ArrayAccessOutOfBounds& m) { m.pc = 5; }, [](ArrayAccessOutOfBounds& m) { m.size = 5; },
         [](ArrayAccessOutOfBounds& m) { m.index = 20; }});

    testMessage<DivisionByZero>([](DivisionByZero& m) { m.pc = 10; }, {[](DivisionByZero& m) { m.pc = 5; }});

    testMessage<EventExecutionKilled>([](EventExecutionKilled& m) { m.pc = 10; },
    ...
    testMessage<NodeSpecificError>(
        ...
        {[](NodeSpecificError& m) { m.pc = 5; }, [](NodeSpecificError& m) { m.message = L"my other error"; }});
    ...
    testMessage<ExecutionStateChanged>(
        ...
        {[](ExecutionStateChanged& m) { m.pc = 5; }, [](ExecutionStateChanged& m) { m.flags = 1; }});
    ...
    testMessage<BreakpointSetResult>(
        ...
        {[](BreakpointSetResult& m) { m.pc = 5; }, [](BreakpointSetResult& m) { m.success = 0; }});

    testMessage<BootloaderReset>([](BootloaderReset& m) { m.dest = 1; }, {[](BootloaderReset& m) { m.dest = 3; }});
    ...
    testMessage<BootloaderReadPage>(
        ...
        {[](BootloaderReadPage& m) { m.dest = 3; }, [](BootloaderReadPage& m) { m.pageNumber = 1; }});
    ...
    testMessage<BootloaderWritePage>(
        ...
        {[](BootloaderWritePage& m) { m.dest = 3; }, [](BootloaderWritePage& m) { m.pageNumber = 1; }});
    ...
    testMessage<BootloaderPageDataWrite>(
        ...
        {[](BootloaderPageDataWrite& m) { m.dest = 3; }, [](BootloaderPageDataWrite& m) { m.data[0] = 4; },
         [](BootloaderPageDataWrite& m) { m.data[1] = 3; }, [](BootloaderPageDataWrite& m) { m.data[2] = 2; },
    ...
    testMessage<SetBytecode>(
        ...
        {[](SetBytecode& m) { m.dest = 3; }, [](SetBytecode& m) { m.bytecode[0] = 3; },
         [](SetBytecode& m) { m.bytecode[1] = 4; }, [](SetBytecode& m) { m.bytecode.push_back(5); }});

    testMessage<Reset>([](Reset& m) { m.dest = 1; }, {[](Reset& m) { m.dest = 3; }});

    testMessage<Run>([](Run& m) { m.dest = 1; }, {[](Run& m) { m.dest = 3; }});

    testMessage<Pause>([](Pause& m) { m.dest = 1; }, {[](Pause& m) { m.dest = 3; }});

    testMessage<Step>([](Step& m) { m.dest = 1; }, {[](Step& m) { m.dest = 3; }});

    testMessage<Stop>([](Stop& m) { m.dest = 1; }, {[](Stop& m) { m.dest = 3; }});

    testMessage<GetExecutionState>([](GetExecutionState& m) { m.dest = 1; },
    ...
    testMessage<BreakpointSet>(
        ...
        {[](BreakpointSet& m) { m.dest = 3; }, [](BreakpointSet& m) { m.pc = 20; }});
    ...
    testMessage<BreakpointClear>(
        ...
        {[](BreakpointClear& m) { m.dest = 3; }, [](BreakpointClear& m) { m.pc = 20; }});

    testMessage<BreakpointClearAll>([](BreakpointClearAll& m) { m.dest = 1; },
    ...
    testMessage<GetVariables>(
        ...
        {[](GetVariables& m) { m.dest = 3; }, [](GetVariables& m) { m.start = 20; },
    ...
    testMessage<SetVariables>(
        ...
        {[](SetVariables& m) { m.dest = 3; }, [](SetVariables& m) { m.start = 20; },
         [](SetVariables& m) { m.variables[0] = 3; }, [](SetVariables& m) { m.variables[1] = 4; },
         [](SetVariables& m) { m.variables.push_back(5); }});

    testMessage<WriteBytecode>([](WriteBytecode& m) { m.dest = 1; }, {[](WriteBytecode& m) { m.dest = 3; }});

    testMessage<Reboot>([](Reboot& m) { m.dest = 1; }, {[](Reboot& m) { m.dest = 3; }});
    testMessage<Sleep>([](Sleep& m) { m.dest = 1; }, {[](Sleep& m) { m.dest = 3; }});

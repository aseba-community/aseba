...
namespace Aseba
{
	...
	void HelpViewer::previousClicked() { viewer->backward(); }

	void HelpViewer::backwardAvailable(bool state) { previous->setEnabled(state); }

	void HelpViewer::nextClicked() { viewer->forward(); }

	void HelpViewer::forwardAvailable(bool state) { next->setEnabled(state); }

	void HelpViewer::homeClicked() { showHelp(HelpViewer::USERMANUAL); }

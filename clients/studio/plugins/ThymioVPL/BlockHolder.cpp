...
namespace Aseba { namespace ThymioVPL
{
	...
	BlockHolder::BlockHolder(const QString& blockType, QGraphicsItem *parent):
	...
	void BlockHolder::paint(QPainter * painter, const QStyleOptionGraphicsItem * option, QWidget * widget)
	{
		...
		QColor color;
		if (blockType == "event")
			color = ThymioVisualProgramming::currentEventColor;
		else
			color = ThymioVisualProgramming::currentActionColor;
		if (!highlight)
		{
			color.setAlpha(130);
	...
	void BlockHolder::addBlock(Block* newBlock)
	{
		...
		polymorphic_downcast<EventActionsSet*>(parentItem())->repositionElements();

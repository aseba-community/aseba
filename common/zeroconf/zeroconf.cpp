...
namespace Aseba
{
	...
	struct ZeroconfCallbacks
	{
		...
		static void DNSSD_API browseReply(DNSServiceRef sdRef, DNSServiceFlags flags, uint32_t interfaceIndex,
			DNSServiceErrorType errorCode, const char* serviceName, const char* regtype, const char* replyDomain,
			void* context);
	...
	void Zeroconf::forget(const std::string& name, const int port) { forget(getTarget(name, port)); }

	//! Forget a target for a given Dashel stream, if the target is unknown, do nothing.
	void Zeroconf::forget(const std::string& name, const Dashel::Stream* stream) { forget(getTarget(name, stream)); }
	...
	void Zeroconf::updateTarget(Zeroconf::Target& target, const TxtRecord& txtrec)
	{
		...
		if (target.state != Target::State::REGISTERED)
			throw Zeroconf::Error(FormatableString("updateTarget: target in invalid state: %0")
									  .arg(target.stateToString.at(target.state)));
	...
	{
		return find_if(
			targets.begin(), targets.end(), [=](const Target& target) { return target.serviceRef == serviceRef; });
	}
	...
	{
		return find_if(targets.begin(), targets.end(), [=](const Target& target) {
			return target.name == name && target.port == port;
		});
	}

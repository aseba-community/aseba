...
namespace Aseba
{
	...
	*/
	...
	bool operator==(
		const TargetDescription::NativeFunctionParameter& lhs, const TargetDescription::NativeFunctionParameter& rhs);
	...
	//! Call the SetBytecode multiple time in order to send all the bytecode
	void sendBytecode(
		std::vector<std::unique_ptr<Message>>& messagesVector, uint16_t dest, const std::vector<uint16_t>& bytecode);
